...
template <class T>
bool NTREE<T>::refreshData(const string& symbol_,const T& dstNode)
{
	T tmpNode;
    tmpNode.init_node(symbol_);
    return refreshData(&tmpNode,dstNode);
}
/**
...
template <class T>
bool NTREE<T>::refreshData_inside(T* srcNode,const T& dstNode)
{
	T *pNodeTmp = search_inside(srcNode);
	if(pNodeTmp==NULL)
	{
		printf("srcNode not found\n" );
		return false;
	}
