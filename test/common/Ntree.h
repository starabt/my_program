...
//user data
	DATAT data;
	// DATAT *pData;
	void set_data(DATAT data_){
		this->data = data_;
	}
	// void set_data(DATAT* pdata_){
	// 	this->pData = pdata_;
	// }
	//inherit class must rewrite this function
	void init_node(const string& symbol_,DATAT data_){
		if(data_)
			this->data = data_;
		this->symbol = symbol_;
	}
...
public:
	...
	bool refreshData(const string& symbol_,const T& dstNode);
...
private:
	bool refreshData_inside(T* srcNode,const T& dstNode);
