...
int main(int argc, char const *argv[])
{
	if(argc != 2)
	{
		printf("Usage: exe <file name to upload>.Pleasee try again\n");
		exit(1);
	}
	sprintf(local_dir,"%s",argv[1]);
    ...
    // unsigned long filesize = 87051748;
    ...
    while(sendLen < filesize)
    {
	    ...
	    len=fread(send_buf,1,sizeof(send_buf),fd);
	    printf("len :::::::::::::%d\n",len );
	    int ret = 0;
	    ret = send(passive_sock,send_buf,len,0);
	    printf("ret::::::::::::%d\n",ret );
	    if(ret <len)
	    {
	    	printf("send not completely\n");
